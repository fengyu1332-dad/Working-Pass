...
async function loadUserStats(userId) {
  ...
  try {
    ...
    const { data: downloads, error: dlErr } = await sb
      ...
      .eq('user_id', userId);
    if (!dlErr) {
      document.getElementById('totalDownloads').textContent = downloads ? downloads.length : 0;
    }
  } catch (error) {
...
async function loadRecentDownloads() {
  ...
  try {
    ...
    if (!downloads || downloads.length === 0) {
      container.innerHTML = '<p style="color:var(--on-surface-variant);text-align:center;padding:24px;">暂无记录</p>';
      return;
    }
    ...
    container.innerHTML = recent.map((d) => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-bottom:1px solid var(--outline);">
        <div>
          <div style="font-weight:600;color:var(--secondary);">${d.reports?.major_name || '专业报告'}</div>
          <div style="font-size:13px;color:var(--on-surface-variant);">${formatDate(d.created_at)}</div>
        </div>
        <div style="font-weight:700;color:var(--error);">-${d.points_spent} 点</div>
      </div>
    `).join('');
  } catch (error) {
