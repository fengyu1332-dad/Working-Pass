...
# 提取MAJORS_DATA
start_idx = content.find('MAJORS_DATA = [')
end_idx = content.find(']')
major_data_str = content[start_idx:end_idx+1]
exec(major_data_str)
...
UNIVERSITY_RECOMMENDATIONS = {
    ...
    "护理学": {"chinese": ["北京协和医学院", "复旦大学护理学院", "中山大学护理学院"], "foreign": ["Johns Hopkins", "U Penn", "King's College London"]},
    ...
    "市场营销": {"chinese": ["北京大学", "中国人民大学", "复旦大学"], "foreign": ["Wharton", "Harvard Business School", "INSEAD"]},
...
def generate_modal_content(major):
    code = major["code"]
    name = major["name"]
    icon = major["category_icon"]
    difficulty = "★" * major["difficulty"] + "☆" * (5 - major["difficulty"])
...
def main():
    ...
    modals = "".join([generate_modal_content(m) for m in MAJORS_DATA])
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .search-input {{ flex: 1; padding: 14px 20px; border: 2px solid var(--outline); border-radius: 16px; font-size: 16px; outline: none; transition: all 0.3s; background: var(--surface-container-low); }}
