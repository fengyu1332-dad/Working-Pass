...
print(f"开始更新 {len(empty_majors)} 个空专业...\n")
...
def generate_content(code, name, category):
    ...
    overview = f"{name}是{category}的重要分支，主要研究专业基础理论和实践应用技术。课程设置包括理论基础、专业核心、实践环节等模块。"
    
    difficulty = "该专业难度较高，需要扎实的数理基础和较强的逻辑思维能力。高年级课程专业性强，需要认真学习和大量练习。"
    
    suitable_for = "适合对专业感兴趣、有志于从事相关行业的学生。要求有较好的学习能力和动手能力。"
    
    xuefeng = f"""{name}是{category}的重要专业：

1. **专业性强**：培养系统专业知识和实践技能，就业竞争力较强。

2. **就业面广**：毕业生可在{career_outlook}等相关领域就业。
...
# 批量更新
def batch_update(major_id, content):
...
for m in empty_majors:
    ...
    print(f"更新: {m['code']} {m['name']}", end="")
    
    if batch_update(m['id'], content):
        success += 1
        print(" ✅")
    else:
        failed += 1
